...
## 04 Wigner-Seitz Method

> [!quote] 核心命题
> 对于碱金属，**孤立原子的电子波函数**和**近自由电子模型的晶体波函数**之间并不矛盾。

### 一、核心思想：桥接两个看似矛盾的世界

**紧束缚方法**从孤立原子出发，**近自由电子法**从平面波出发，但两者似乎互相矛盾：碱金属（如钠）的导带电子，到底是更像原子轨道还是更像自由电子的平面波？

Wigner 和 Seitz 给出的答案是：**两者其实并不矛盾**。

导带电子在**离子实附近**表现得像原子轨道，但在**原子之间的广阔空间**里表现得像平面波。Wigner-Seitz 方法的目标就是构造一个在整个晶胞内都自洽的波函数，同时兼顾这两端。
...
$$\left(\frac{1}{2m}\mathbf{p}^2 + U(\mathbf{r})\right) e^{i\mathbf{k}\cdot\mathbf{r}} u_{\mathbf{k}}(\mathbf{r}) = \epsilon_{\mathbf{k}} e^{i\mathbf{k}\cdot\mathbf{r}} u_{\mathbf{k}}(\mathbf{r})$$

利用动量算符的性质 $\mathbf{p}(e^{i\mathbf{k}\cdot\mathbf{r}}f) = e^{i\mathbf{k}\cdot\mathbf{r}}(\mathbf{p} + \hbar\mathbf{k})f$，经过运算可重写为：

$$\boxed{\left(\frac{1}{2m}(\mathbf{p} + \hbar\mathbf{k})^2 + U(\mathbf{r})\right) u_{\mathbf{k}}(\mathbf{r}) = \epsilon_{\mathbf{k}} u_{\mathbf{k}}(\mathbf{r})} \tag{17}$$

这是一个**只关于周期性函数 $u_{\mathbf{k}}(\mathbf{r})$ 的方程**，其中多出了 $\hbar\mathbf{k}$ 这个参数。

### 三、关键技巧：先解 $\mathbf{k} = 0$，再推广

当 $\mathbf{k} = 0$ 时，方程退化为：

$$\boxed{\left(\frac{1}{2m}\mathbf{p}^2 + U(\mathbf{r})\right) u_0(\mathbf{r}) = \epsilon_0 u_0(\mathbf{r})} \tag{19}$$

这有一个巨大的好处：**$u_0(\mathbf{r})$ 具有晶格的完整对称性**，是一个非简并基态，没有额外的相位因子干扰，求解远比一般 $\mathbf{k}$ 值容易。

求出 $u_0(\mathbf{r})$ 之后，Wigner-Seitz 的**核心构造**是：对任意 $\mathbf{k}$，近似取

$$\boxed{\psi_{\mathbf{k}}(\mathbf{r}) = e^{i\mathbf{k}\cdot\mathbf{r}} u_0(\mathbf{r})} \tag{18}$$

这个波函数天然就是 Bloch 形式，且 $u_0(\mathbf{r})$ 已经包含了离子实势场的全部信息。

> [!note] 近似程度
> 这**不是**精确解——方程 (17) 中有一个 $\hbar\mathbf{k}\cdot\mathbf{p}/m$ 项被忽略了。在教材习题8中，这个项被作为微扰处理，这正是 $k\cdot p$ 微扰理论的内容，对计算能带边缘的有效质量非常有用。

### 四、Wigner-Seitz 边界条件

这是整个方法**最巧妙的地方**。

| | 孤立原子 | 晶体（Wigner-Seitz） |
|:---|:---|:---|
| **边界条件** | $\psi(r) \to 0$ ($r \to \infty$) | $(d\psi/dr)_{r_0} = 0$ |
| **物理含义** | 波函数在无穷远处衰减为零 | 在两原子中点处，法向导数为零（对称性要求） |

其中 $r_0$ 是**与一个原胞等体积的球的半径**。

以钠为例：$r_0 = 3.95$ 原子单位（约 2.08 A），而钠原子到最近邻距离的一半只有 1.86 A。这个"中点球"比原子实际尺寸稍大，但对 fcc 和 bcc 结构，这种球形近似尤其合理。

> [!important] 这个边界条件的物理后果
> 它允许基态波函数的**曲率远小于**孤立原子中的曲率。曲率小意味着**动能小**——这正是晶体中原子价电子能量降低的根本原因。

### 五、钠原子——实际成果

教材图19展示了钠 3s 轨道的径向波函数对比：

| 状态 | 边界条件 | 能量 |
|------|---------|------|
| 孤立 Na 原子 3s | $\psi(r) \to 0$ ($r \to \infty$) | $-5.15$ eV |
| 金属 Na ($\mathbf{k} = 0$) | $(d\psi/dr)_{r_0} = 0$ | $-8.2$ eV |

**仅仅因为边界条件的改变，电子能量就下降了 3.05 eV。**

再看整个导带的平均能量：
- $\mathbf{k}=0$ 处能量：$-8.2$ eV
- 费米能量：$3.1$ eV
- 每个电子的平均动能：$0.6 \times E_F = 1.9$ eV
- **平均电子总能量**：$-8.2 + 1.9 = -6.3$ eV

与孤立原子（$-5.15$ eV）相比，降低约 **1.1 eV**，与实验测得的钠的结合能 **1.13 eV** 高度吻合。

### 六、$u_0(\mathbf{r})$ 的物理图像

图19揭示了 $u_0(\mathbf{r})$ 的一个有趣特征：

> **在原子体积的约 90% 的区域内，它几乎是常数。** 只有在靠近离子实（ion core）的区域，波函数才急剧增长并产生振荡。

这就解释了为什么 Wigner-Seitz 方法能够统一"原子轨道"和"近自由电子"两种看似矛盾的图像：

- **在离子实附近（约 10% 体积）**：波函数有强烈的调制和振荡，表现得像原子轨道
- **在离子实之间的广阔区域（约 90% 体积）**：波函数几乎是平坦的平面波

### 七、与紧束缚方法的对比

| 维度 | 紧束缚方法 | Wigner-Seitz 方法 |
|------|----------|------------------|
| **出发点** | 孤立原子轨道 $\varphi(\mathbf{r}-\mathbf{r}_j)$ | 晶格周期势中的 Bloch 方程 |
| **波函数形态** | 局域在格点附近 | 扩展在整个晶格 |
| **核心近似** | 只保留同一原子和最近邻的积分 | 先解 $\mathbf{k}=0$，再忽略 $\mathbf{k}\cdot\mathbf{p}$ 项 |
| **能量色散** | $\epsilon_\mathbf{k} = -\alpha - \gamma\sum e^{-i\mathbf{k}\cdot\boldsymbol{\rho}_m}$ | $\epsilon_\mathbf{k} \approx \epsilon_0 + \hbar^2 k^2/2m$ |
| **适用对象** | 内层电子、d 带、共价键 | 碱金属等简单金属的导带 |
| **物理直觉** | "从一个格点跳到另一个格点" | "大部分时间在间隙漫游，靠近核才被扰动" |

```mermaid
flowchart LR
    A[孤立原子] --> B[紧束缚法<br/>原子轨道线性组合]
    A --> C[Wigner-Seitz法<br/>周期势中求解基态]
    B --> D[窄能带<br/>局域电子]
    C --> E[近自由色散<br/>扩展电子]
    D -.->|同一薛定谔方程| E
```

> [!abstract] Wigner-Seitz 方法核心要点
>
> 1. **出发点**：晶格周期势中的 Bloch 方程，先求解 $\mathbf{k}=0$ 的周期函数 $u_0(\mathbf{r})$
> 2. **核心构造**：$\psi_\mathbf{k}(\mathbf{r}) = e^{i\mathbf{k}\cdot\mathbf{r}} u_0(\mathbf{r})$，天然满足 Bloch 条件
> 3. **关键技巧**：Wigner-Seitz 边界条件 $(d\psi/dr)_{r_0} = 0$，允许波函数曲率减小 → 动能降低
> 4. **能量色散**：$\epsilon_\mathbf{k} \approx \epsilon_0 + \hbar^2 k^2/2m$（自由电子形式，但 $\epsilon_0$ 因晶格势降低）
> 5. **统一图像**：离子实附近像原子轨道（2s 导带需与 1s、2s 正交，产生节点振荡），间隙处像平面波
> 6. **对结合能的解释**：边界条件改变 → 波函数曲率减小 → 动能降低，解释了钠 1.13 eV 的结合能

---

## 05 Pseudopotential Methods

> [!quote] Hellmann & Kassatotschkin (1936)
> *"Since the field of the ion determined in this way runs a rather flat course, it is sufficient in the first approximation to set the valence electron in the lattice equal to a plane wave."*

### 一、为什么要引入赝势？

想象一个钠原子：内层有 1s、2s、2p 共 **10 个芯电子**，外层只有 **1 个 3s 价电子**。

当钠形成金属晶体时，10 个内层电子几乎不变，真正参与成键、导电的只有那一个 3s 价电子。但直接求解薛定谔方程会遇到巨大的麻烦：

**价电子波函数在离子实区域内必须与所有芯电子正交。** 以钠的 3s 导带电子为例，它必须同时与 1s（无节点）和 2s（一个节点）正交。这意味着 3s 导带电子在离子实区域内**至少有两个节点**——波函数剧烈振荡。

> [!question] 核心问题
> 我们为什么要为一个在核心区域剧烈振荡、但对外部行为几乎无影响的波函数，去求解一个极其复杂的强势场？

**赝势方法的答案**：用一个人造的、弱得多的势（"赝势"）替换真实的强离子实势场，使得赝势下求解出的波函数在核心外部与真实波函数**完全相同**，但在核心内部被**"平滑化"**，不再剧烈振荡。

> **保留你需要的（外部行为），去掉你不需要的（内部振荡）。**

### 二、物理图像：核心区域的"无谓复杂"

| 区域 | 真实势场 | 波函数 | 物理行为 |
|------|---------|--------|---------|
| **离子实外部** | 很弱（~ $-e^2/r$，被屏蔽） | 几乎平面波 | 能量 $\epsilon_\mathbf{k} \approx \hbar^2 k^2/2m$ |
| **离子实内部** | 极强（钠 r=0.15 A 处约 -50.4 eV） | 剧烈振荡，多个节点 | 对能带 $k$ 依赖关系**基本无关** |

Kittel 给出了一个精彩论证：

> *"What goes on in the core is largely irrelevant to the dependence of $\epsilon$ on k. 在外部区域应用 Hamiltonian 算符，会得到几乎等于自由电子的能量。"*

### 三、Cancellation Theorem（抵消定理）

赝势最惊人的发现：**那个能够给出相同外部波函数的有效势，其强度几乎为零。**

真实势场在核心区域由两部分组成：
1. 核的强吸引势（$-Ze^2/r$，Z=11）
2. 芯电子的强排斥势（交换和相关作用）

这两部分**方向相反、几乎完全抵消**，使得净效果远小于任何一个单独的贡献。

教材图22a 直观展示了这一抵消：

| r (A) | 真实势场 | 赝势 | 比值 |
|:---:|:---:|:---:|:---:|
| 0.15 | -50.4 eV | ~ 0 | **~200 倍** |
| 0.40 | -11.6 eV | ~ 0 | **很大** |
| 0.70 | -4.6 eV | ~ 0 | **很大** |

### 四、Empty Core Model（空心核模型）

最简单的赝势模型，极其直观：

$$\boxed{U(r) = \begin{cases} 0, & r < R_e \\ -e^2/r, & r > R_e \end{cases}} \tag{21}$$

在 $r < R_e$ 的内部，势为零（"空心核"）。在 $r > R_e$ 的外部，势就是裸的离子库仑势。

关键参数 $R_e$ 是**空核半径**，通过匹配实验数据确定。对钠金属，$R_e = 1.66 a_0$。

然后裸势还需要被**屏蔽**。用电子气的介电函数 $\epsilon(\mathbf{K})$ 做屏蔽：

$$U_{\text{screened}}(\mathbf{K}) = \frac{U(\mathbf{K})}{\epsilon(\mathbf{K})}$$

图22a 中使用的就是 Thomas-Fermi 介电函数做屏蔽。

### 五、倒格空间中：$U(\mathbf{G})$ 的意义

能带结构计算依赖于**赝势在倒格矢 $\mathbf{G}$ 处的傅里叶分量 $U(\mathbf{G})$**：

$$U(\mathbf{G}) = \int_{\text{cell}} U(\mathbf{r}) e^{-i\mathbf{G}\cdot\mathbf{r}} d\mathbf{r}$$

两个重要结论：

1. **通常只需少数几个 $U(\mathbf{G})$ 就能得到极好的能带结构**。
2. **$U(0)$ 有第一性原理估计**：对于屏蔽库仑势，$U(0) = -\frac{2}{3} E_F$。

> [!tip] 与近自由电子模型的连接
> 在近自由电子模型中，晶格势的傅里叶分量 $U(\mathbf{G})$ 决定了能隙的大小。赝势法中的 $U(\mathbf{G})$ 正是这个物理量，只不过它已经"去芯"了——弱得多，但保留了对外部波函数的全部影响。
...
| 步骤 | 内容 |
|------|------|
| 1 | 从光学反射和吸收实验中反推出 $U(\mathbf{G})$ 的少数几个值 |
| 2 | 用这些 $U(\mathbf{G})$ 计算能带结构 |
| 3 | 计算结果与实验高度吻合 |

EPM 的 $U(\mathbf{G})$ 通常具有**可加性**——不同种类离子的贡献可以分别确定后再叠加。从已知结构"借用" $U(\mathbf{G})$ 值可以预测全新结构的能带。

#### 6.2 第一性原理赝势计算（Ab Initio）
...
教材引用的 Yin 和 Cohen 对比数据：

| 材料 | 晶格常数 计算(A) | 实验(A) | 结合能 计算(eV) | 实验(eV) | 体模量 计算(Mbar) | 实验(Mbar) |
|------|:---:|:---:|:---:|:---:|:---:|:---:|
| **硅** | 5.45 | 5.43 | 4.84 | 4.63 | 0.98 | 0.99 |
| **锗** | 5.66 | 5.65 | 4.26 | 3.85 | 0.73 | 0.77 |
| **金刚石** | 3.60 | 3.57 | 8.10 | 7.35 | 4.33 | 4.43 |

**三组参数在所有三种材料中，计算值与实验值的偏差都小于一。**
...
flowchart TD
    subgraph 能带理论的三种路径
    direction TB
    A[孤立原子] --> B[紧束缚法] --> C[原子轨道线性组合]
    A --> D[Wigner-Seitz法] --> E[周期势中求解基态]
    A --> F[赝势法] --> G[有效势 + 平面波]
    C --> H[能带结构]
    E --> H
    G --> H
...
| 维度 | 紧束缚法 | Wigner-Seitz 法 | 赝势法 |
|------|--------|----------------|--------|
| **出发点** | 原子轨道 $\varphi(\mathbf{r}-\mathbf{r}_j)$ | 周期势中 $u_0(\mathbf{r})$ | 平滑化的有效势 |
| **对核心区域** | 原子轨道描述 | 需要精确求解 | 直接忽略/平滑化 |
| **波函数形态** | 局域在格点 | 扩展，核心处振荡 | 扩展，核心处平滑 |
| **适用对象** | 内层电子、d 带 | 碱金属 s 电子 | 简单金属、半导体 |
| **对结合能的解释** | 能级分裂 → 占据低能态 | 边界条件改变 → 动能降低 | 抵消定理 → 弱有效势 |

### 八、现代赝势的发展（Beyond Kittel）

虽然 Kittel 只讨论了最简单的 Empty Core Model 和经验赝势方法，但赝势理论在现代计算材料科学中已经发展出丰富体系：

| 类型 | 提出者 | 核心思想 |
|------|--------|---------|
| **Norm-Conserving PP** | 早期方案 | 赝波函数在截断半径 $R_c$ 外与真实波函数一致，且内部积分（norm）也一致 |
| **Ultrasoft PP (USPP)** | Vanderbilt, 1990 | 放松 norm 守恒条件，允许赝波函数在核心区域更"软"，大幅减少平面波基组 |
| **PAW** | Kresse & Joubert, 1999 | 在赝波函数和真实波函数之间建立线性变换，既保留赝势效率，又能恢复真实波函数 |

在现代 DFT（密度泛函理论）计算中，赝势是**必不可少的工具**。没有赝势，直接求解含芯电子的全势薛定谔方程需要数千甚至数万平面波；有了赝势，通常几十到几百个平面波就足够了。

### 九、赝势的物理哲学

> **在固体物理中，很多看似复杂的物理现象——离子实的强库仑势、芯电子的交换关联、波函数在核心区域的剧烈振荡——对外围价电子的行为影响微乎其微。用一个简单得多的有效势来取代它们，不仅计算效率大幅提升，物理图像反而更加清晰。**

> [!abstract] 赝势法核心要点
>
> 1. **出发点**：价电子波函数在离子实区域内剧烈振荡（与芯电子正交的要求），但核心区域对外部行为基本无关
> 2. **核心思想**：用弱的有效势（赝势）替换强的真实势场，使得外部波函数不变，内部被平滑化
> 3. **Cancellation Theorem**：真实势场中核吸引和芯电子排斥几乎完全抵消，净效果很小
> 4. **Empty Core Model**：最简单的赝势模型——$r < R_e$ 时 $U=0$，$r > R_e$ 时 $U = -e^2/r$
> 5. **倒格空间**：能带计算只需少数几个 $U(\mathbf{G})$ 傅里叶分量
> 6. **经验赝势方法 (EPM)**：从光学实验反推 $U(\mathbf{G})$，计算能带与实验高度吻合
> 7. **第一性原理赝势**：仅需晶体结构 + 原子序数，就能高精度预测晶格常数、结合能、体模量
> 8. **物理哲学**：强势 -> 弱赝势 -> 近似平面波。每一步简化都不丢失关键物理，却换来计算效率的巨大飞跃
