...
`dynamic_cast` is the only casting operator in C++ that **performs type checking at runtime**, primarily used for **safe downcasting** or **cross-casting** in polymorphic types (i.e., class hierarchies with virtual functions).


| Scene             | Instructions                                                                                                                      |
| ----------------- | --------------------------------------------------------------------------------------------------------------------------------- |
| **downcasting**   | Convert from **base class pointer/reference** to **derived class pointer/reference**                                              |
| **cross-casting** | In **multiple inheritance**, convert from **one base class** pointer to **another base class** pointer                            |
| **Safe check**    | If the **conversion fails**, the pointer **returns nullptr** , and the **reference throws a** **`std::bad_cast`** **exception** . |

- An example to show the successful **DownCasting** :
...
class Base {
public:
    virtual ~Base() {}  // dynamic_cast work when there is a virtual function
};
...
int main() {
    Base* base = new Derived();  // up cast is implicit

    // downcasting
    Derived* derived = dynamic_cast<Derived*>(base);
    if (derived) {
        derived->sayHello();  // succeed
    } else {
...
Derived* derived = dynamic_cast<Derived*>(base);
if (!derived) {
    std::cout << "Cast failed! base is not a Derived." << std::endl;
}
```
...
- An example to show the **Cross-Casting** 
...
int main() {
    A* a = new C();
    B* b = dynamic_cast<B*>(a);  // cross-casting
    if (b) {
...
The Condition to use the `dynamic_cast` :

| Condition                | Instructions                                                                                          |
| ------------------------ | ----------------------------------------------------------------------------------------------------- |
| **Polymorphic type**     | The class must have **virtual functions** (usually a virtual destructor).                             |
| **Pointer or reference** | Can only be used for pointer or reference types                                                       |
| **Runtime check**        | When it **fails**, the **pointer returns** `nullptr` , and the **reference** throws `std::bad_cast` . |
